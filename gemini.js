...
const GPT_MEMBER_ID =
  PropertiesService.getScriptProperties().getProperty("GPT_BOT_MEMBER_ID");
  ...
  return slackMsgs.map((msg) => {
    ...
    if (files && files.length > 0) {
      files.forEach((imageUrl) => {
        ...
        const mimeType = getMimeType(imageUrl.split(".").pop());
        if (!mimeType) return;
        parts.push({
          inline_data: {
            mime_type: mimeType,
...
const fetchAIAnswerText = (tiggerMsg) => {
  ...
  // 取得したメッセージをGemini用に変換する
  const msgsForGemini = parseSlackMsgsToGeminiQuesryMsgs(msgsAskedToBot);
  ...
  try {
    ...
    if (resPayloadObj.candidates[0].content.parts[0].text === 0)
      return "AIからの応答が空でした";
